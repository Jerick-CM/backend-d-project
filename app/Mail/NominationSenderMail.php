...
class NominationSenderMail extends Mailable
{
     ...
     * @return void
     */
    public function __construct($user, $recipient, $message, $badge, $token, $date, $time, $isPreview = false,$editpreview,$content)
    {
        ...
        $this->preheadertext3 = str_replace("{ NAME }", $user->name ,$EdmHeader->preheadertext3);
        ...
        $edmType = $token ? Edm::TYPE_NOMINATION_SENDER_TOKEN : Edm::TYPE_NOMINATION_SENDER;

        if($token){

            $EdmTemplateBody_id = 3;
            ...
            if($editpreview){
                ...
                $this->contentbody = str_replace("{{recipientName}}", $this->recipientName ,$this->contentbody); 
            ...
            $email_category = "Message Send with Token";

        }else{

            $EdmTemplateBody_id = 1;
            $EdmTemplateBody = EdmTemplateBody::find($EdmTemplateBody_id);   
            ...
            $email_category = "Message Send";
       

        }

        // if (! $isPreview) {
        //     event(new AdminLogEvent($user->id, AdminLog::TYPE_EDM_SENT_EMAIL, [
        //         'sender_id' => $user->id,         
        ...
        if (!$isPreview) {
            $edmLog = EdmLog::create([
                'type' => Edm::TYPE_NOMINATION_SENDER,
                'data' => [
                    'user' => $user,
                    'recipient' => $recipient,
                    'message' => $message,
                    ...
                    'date' => $date,
                    'time' => $time,
                    'subject'=> __('edm.sent_message.title')
                ],
    ...
    {
        // return $this->subject(__('edm.sent_message.title'))
