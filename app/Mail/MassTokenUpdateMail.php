...
class MassTokenUpdateMail extends Mailable
{
    ...
    public function __construct(  $user,$email,$date,$isPreview = false, $EdmTemplateBody_id,$admin,$message,$RO_deduct,$RO_add,$MR_deduct,$MR_add,$editpreview,$content)
    {
        ...
        $data_url["domain"] = $_SERVER['SERVER_NAME'];
          switch ($data_url["domain"]){
            case 'deloitte-backend.local.nmgdev.com':
                ...
                // $this->image = url('img/edm/welcome-banner-masstokenupdate.jpg');
                break; 

            default:
                // $this->image1 = secure_url($EdmHeader->image1);
                $this->image2 = secure_url($EdmHeader->image2);
                // $this->image = secure_url('img/edm/welcome-banner-masstokenupdate.jpg');
        }
        ...
        $this->masstokenupdatetable = view('edm.masstokenupdate.masstokenupdatetable')
                                                ->with('RO_deduct', $RO_deduct)
                                                ->with('RO_add', $RO_add)
                                                ->with('MR_deduct', $MR_deduct)
                                                ->with('MR_add', $MR_add);
        ...
        if($editpreview){
            ...
            $this->bodymessage = str_replace("{{name}}", $user->name , $this->bodymessage);
            $this->bodymessage = str_replace("{{message}}", $message , $this->bodymessage);  
            $this->bodymessage = str_replace("{{masstokenupadetable}}", $this->masstokenupdatetable , $this->bodymessage); 


            $this->bodymessage =  str_replace("<p>", "" ,$this->bodymessage); 
            $this->bodymessage =  str_replace("</p>", "<br>" ,$this->bodymessage); 
            
        }else{

            $this->bodymessage = str_replace("{{name}}", $user->name , $EdmTemplateBody->content);
            $this->bodymessage = str_replace("{{message}}", $message , $this->bodymessage);  
            $this->bodymessage = str_replace("{{masstokenupadetable}}", $this->masstokenupdatetable , $this->bodymessage); 
        ...
        $this->tpl = 'edm.masstokenupdate.default';

         if (! $isPreview) {

            $edmLog = EdmLog::create([
                'type' => Edm::TYPE_MASSTOKENUPDATE_EMAIL,
                'data' => [
                    'user' => $user,  
